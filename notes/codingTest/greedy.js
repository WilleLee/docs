...
function bigNumLaw(list, m, k) {
    ...
    var _a = list.sort(function (a, b) { return b - a; }), first = _a[0], second = _a[1];
    var sum = 0;
    var count = 0;
    while (count < m) {
        count++;
...
function bigNumLaw2(list, m, k) {
    ...
    var _a = list.sort(function (a, b) { return b - a; }), first = _a[0], second = _a[1];
    var sum = 0;
    var firstCount = Math.floor(m / (k + 1)) + (m % (k + 1));
    var secondCount = m - firstCount;
...
function cardGame(list) {
    ...
    var minList = list.map(function (arr) { return Math.min.apply(Math, arr); });
    console.timeEnd("cardGame");
    return Math.max.apply(Math, minList);
}
