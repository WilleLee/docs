...
리듀서는 컴포넌트의 상태 업데이트 로직을 통합하도록 하며, 컨텍스트는 정보를 다른 컴포넌트로 깊이 내려보낼 수 있도록 한다. 이러한 리듀서와 컨텍스트를 결합하면 복잡한 화면의 상태를 관리할 수 있다.

### 리듀서와 컨텍스트 결합하기

리듀서를 적용했던 예시를 다시 가져와보면, 리듀서 함수는 모든 상태 업데이트 로직을 포함하며, 파일의 하단에서 선언되고 있다.
...
리듀서는 이벤트 핸들러를 간결하고 정확하게 유지하는 데 도움을 주지만, 앱이 커질수록 다른 어려움에 봉착하게 한다. **위에서 `tasks` 상태와 `dispatch` 함수는 `<TaskApp>` 컴포넌트의 최상단에서만 사용 가능하다.** 다른 컴포넌트가 `tasks`를 읽고 변경할 수 있게 하려면 현재 상태와 이벤트 핸들러를 props로 내려보내야 하는 것이다.

가령 아래를 보면 `<TaskApp>`이 `<TaskList>`로 `tasks`와 이벤트 핸들러를 전달하며:
...
`<TaskList>`는 `<Task>`로 이벤트 핸들러를 전달한다.

```javascript
<Task task={task} onChange={onChangeTask} onDelete={onDeleteTask} />
```

이렇게 하는 것은 물론 작동에 문제가 없으나, 중간에 상당히 많은 수의 컴포넌트가 끼어 있다면 모든 상태와 함수를 내려보내는 일은 문제가 될 것이다.

이 때문에 위 `tasks` 상태와 `dispatch` 함수를 컨텍스트에 넣는 것을 하나의 대안으로 생각하게 된다. **이로써 트리 상 `<TaskApp>` 아래에 있는 모든 컴포넌트가 반복적인 "prop 드릴링" 없이 `tasks`를 읽고 액션을 전달(디스패치)할 수 있게 된다.**

리듀서와 컨텍스트의 결합은 아래 단계에 걸쳐 이루어진다:

1. 컨텍스트를 **생성한다**.
2. 상태와 디스패치를 컨텍스트에 **넣는다**.
3. 트리 내 원하는 곳에서 컨텍스트를 **사용한다**.

#### 스텝 1. 컨텍스트 생성

`useReducer` 훅은 현재의 `tasks`와 이를 업데이트하도록 하는 `dispatch` 함수를 반환한다.

```javascript
const [tasks, dispatch] = useReducer(tasksReducer, initialTasks);
```

이들을 트리 아래로 전달하기 위해 두 개의 서로 다른 컨텍스트를 생성해야 한다.

- `TasksContext`는 현재 `tasks` 값을 제공한다.
- `TasksDispatchContext`는 컴포넌트로 하여금 액션을 전달(디스패치)하게 하는 함수를 제공한다.

이들을 export함으로써 다른 파일에서 추후 import할 수 있게 한다.
...
여기선 디폴트 값으로 `null`을 전달하는데, 실제 값은 `<TasksApp>` 컴포넌트에 의해 제공될 것이다.

#### 스텝 2. 상태 및 디스패치를 컨텍스트에 삽입

이제 `<TasksApp>` 컴포넌트에서 두 컨텍스트 모두를 import 할 수 있다. `useReducer()`가 반환하는 `tasks`와 `dispatch`를 하위 트리 전체에 제공하면 된다.
...
#### 스텝 3. 트리 내 원하는 곳에서 컨텍스트 사용

이제 `tasks`나 이벤트 핸들러를 트리 아래로 전달할 필요가 없으므로 prop 전달을 제거하고 `tasks` 값을 필요로 하는 컴포넌트에서 `TasksContext`에 저장된 값을 읽도록 하면 된다.
...
`tasks`를 업데이트하려면 어떤 컴포넌트든지 컨텍스트의 `dispatch` 함수를 읽어 호출하게 하면 된다.
...
**`<TaskApp>` 컴포넌트는 어떤 이벤트 핸들러도 내려보내지 않으며, `<TaskList>` 또한 `<Task>` 컴포넌트에 이벤트 핸들러를 전달하지 않는다.** 각 컴포넌트는 단지 필요한 컨텍스트를 읽을 뿐이다.

**상태는 여전히 `<TaskApp>` 컴포넌트 최상단에 "살며", `useReducer`에 의해 관리된다.** 그러나 상태의 `tasks`와 `dispatch`는 이제 해당 컨텍스트를 사용하면 트리 아래의 어떤 컴포넌트에서든 접근 가능하다.

### 모든 관계성을 하나의 파일로 옮기기

필수적인 건 아니지만, 리듀서와 컨텍스트를 하나의 파일로 옮겨 컴포넌트를 더 정리해볼 수 있다. `TasksContext.js` 파일은 현재 두 개의 컨텍스트 선언만을 포함한다:
...
이제 같은 파일로 리듀서를 옮기고, 또 여기에 새로운 `<TasksProvider>` 컴포넌트를 선언하여 아래의 모든 조각들을 한 데 묶도록 만들 것이다:

1. 리듀서를 이용한 상태 관리
2. 두 컨텍스트 모두를 하위 컴포넌트에 제공
3. `children`을 prop으로 취하여 JSX를 전달하도록 허용
...
**이러한 작업은 기존 `<TaskApp>` 컴포넌트의 모든 복잡한 배선을 제거한다.**
...
또한 해당 컨텍스트를 **사용하는** 함수들을 같은 `TasksContext.js` 파일에서 export할 수도 있다.
...
그러면 컴포넌트는 이 함수들을 통하여 컨텍스트를 읽을 수 있게 된다.
...
**이제 모든 컨텍스트와 리듀서의 배선이 `TasksContext.js`에 존재한다. 이는 컴포넌트를 깨끗하게 유지하여 컴포넌트가, 어디에서 데이터를 얻는가가 아니라, 무엇을 보여줘야 하는지에 집중하게 한다.**

여기서 `<TasksProvider>`는 `tasks`를 다루는 방법을 알고 있는 화면의 한 부분과 같고, `useTasks`는 `tasks`를 읽는 방법과 같으며, `useTasksDispatch`는 트리 상 하위의 어떤 컴포넌트에서든 `tasks`를 업데이트할 수 있는 방법과 같다.
